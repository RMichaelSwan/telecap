...
"""
    ...
    TODO: Move CV box acquisition code to proper functions/modules
...
def main():
    gallery_color = np.array([26,26,26,255]) #zoom gallery color in RGB
    active_color = np.array([35,217,89,255]) #zoom gallery color in RGB

    x = WindowCapture()
    img_arr = x.get_numpy_img()
    print(img_arr.shape)
    img_arr = img_arr[0:int(img_arr.shape[0]*0.89),0:int(img_arr.shape[1]*0.99),...] #crop out extra bars around window edges
    print(img_arr.shape)
    img = array_to_cv2(img_arr,cv2.COLOR_RGB2BGR)
    masked = img_arr.copy()
    
    gal_mask = (img_arr == gallery_color).all(-1)
    
    masked[gal_mask] = 255
    masked[np.logical_not(gal_mask)] = 0
    
    ##We can find the active partipant using the color...
    # active_mask = (img_arr == active_color).all(-1)
    ...
    for cnt in contours:
        ...
        if len(approx)==4: #rectangles have 4 sides
            ...
            if (w/h > 5): #bad aspect ratio
                # print(f"rejected aspect ratio of {w/h}")
                continue
    ...
    print(f"Detected {len(rects)} participants.")
